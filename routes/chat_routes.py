...
from services import llm, prompt_spec
...
def chat(req: ChatRequest) -> dict[str, Any]:
    """텍스트는 deepseek로, 이미지가 있으면 먼저 비전 모델로 분석 후 텍스트 모델이 응답."""
    ...
    for msg in req.messages:
        if msg.role == "user" and msg.images:
            ...
            try:
                analysis = llm.analyze_image(msg.images, note=msg.content)
                vision_used = True
            except Exception as exc:  # noqa: BLE001
                analysis = f"(이미지 분석 실패: {llm._friendly_error(exc)})"
    ...
    try:
        reply = llm.chat_reply(history)
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "error": llm._friendly_error(exc)}
